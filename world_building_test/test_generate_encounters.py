...
class TestGenerateEncounters(TestCase):
    ...
    def test_generate_encounters_health(self, _):
        actual = generate_encounters()
        self.assertEqual('health', actual)

    @patch('random.randint', return_value=2)
    def test_generate_encounters_damage(self, _):
        actual = generate_encounters()
        self.assertEqual('damage', actual)

    @patch('random.randint', return_value=3)
    def test_generate_encounters_defence(self, _):
        actual = generate_encounters()
        self.assertEqual('defense', actual)

    @patch('random.randint', return_value=4)
    def test_generate_encounters_puzzle(self, _):
        actual = generate_encounters()
        self.assertEqual('puzzle', actual)

    @patch('random.randint', return_value=5)
    def test_generate_encounters_riddle(self, _):
        actual = generate_encounters()
        self.assertEqual('riddle', actual)

    @patch('random.randint', return_value=6)
    def test_generate_encounters_battle(self, _):
        actual = generate_encounters()
